...
async function toggleGyro() {
  ...
  if (!gyroEnabled) {
    ...
    // Prefer native deviceorientation on mobile so we can calibrate yaw/pitch
    // against the current screen orientation explicitly.
    if (canUseDeviceOrientation) {
      ...
      gyroFallbackListener = (event) => {
        ...
        const currentYawDeg = ((currentScene.view.parameters().yaw || 0) * 180) / Math.PI;
